...
- **Extra income**: `+ Add bonus` button per income row opens sub-rows with description + amount fields. Multiple extras per source per month allowed. Saved to `extra_income` table via `clear_extra_income()` + `add_extra_income()` on each save. Dashboard income card shows "+€X in extras" sub-line when `extra_total > 0`. **Pattern for summing income + extras**: always use `sum(si.values() if si else []) + sum(e["amount"] for e in ex)` — never `(sum(si.values()) + sum(extras)) if si else 0.0`, which silently drops extras when `si` is empty.
...
- **Portfolio value on save**: after `save_snapshot()`, the current portfolio EUR value is calculated from `get_portfolio_positions()` + `get_portfolio_cache()` and stored via `update_snapshot_portfolio(year, month, portfolio_eur)` — **only when saving the current calendar month** (no historical overwrite) and **only when `port_total_eur > 0`** (never zeros out an existing good value). Wrapped in try/except so missing cache never blocks the save.
...
- **Formula (constant, does not change with passing days)**:
  - `estimated_eom = latest_nw + expected_income - (last_day × daily_buffer) - fx_total`
  - `expected_income`: if the user has already entered income for the current month via the snapshot income section (`get_snapshot_income(today.year, today.month)` is non-empty), that recorded amount + any extras is used. Otherwise falls back to last month's recorded income + extras as a proxy (`get_snapshot_income(latest.year, latest.month)`). Zero if neither exists.
- **Dashboard card** ("END-OF-MONTH ESTIMATE"): shows "Latest net worth", "Expected income (current month)" or "Expected income (est. from MMM YYYY)" in GREEN, "Daily allowance (N days × €X/day)" in RED using full month days (constant), "Fixed expenses this month" in RED, then "Estimated end-of-month net worth". No Edit button for daily allowance here.
...
### Dashboard
- **Metric cards**: NET WORTH, MONTHLY CHANGE, FIXED EXPENSES (3 static cards in `cards_frame`). Extra cards rendered by `_render_extra_cards()` into `_extra_cards_frame`: PORTFOLIO + ALLOCATION DONUT (conditional), LAST MONTH INCOME (conditional). LAST MONTH INCOME shows income from `latest` (snapshots[0]); pct-change compares to `prev_snap` (snapshots[1]); label reads "vs {MONTHS[prev_snap['month']-1]}". `has_income_data = bool(latest)`. **Monthly change uses portfolio-inclusive totals with 0→value guard**: `_render_comparison()` only adds `portfolio_eur` to both sides when *both* snapshots have `portfolio_eur > 0`; falls back to cash-only comparison otherwise — avoids artificial spike when portfolio is first populated. AVG/MO uses `_change_guarded(a, b)` helper (same guard) divided by **elapsed calendar months** between first and last snapshot (`(last.year - first.year) * 12 + (last.month - first.month)`) — not snapshot count — so gaps in history don't inflate the average. Sparkline bars use `_snap_change(a, b)` helper with the same guard.
...
- **Monthly Net Worth Change** (bar chart) — ≥2 snapshots; green/red bars; `ax.margins(y=0.30)`; stats header (latest change + comparison to previous). **Uses portfolio-inclusive totals** via module-level `_snap_total_incl(s)` helper (`s["total"] + portfolio_eur`); consistent with Dashboard Monthly Change card.
- **Cash Flow** (income bar + spending bar + net savings line) — spending bar shown for ANY month where a previous snapshot exists (`key in prev_nw`). First snapshot never gets a spending bar (no prev). `spent_per_month: list[float | None]` (None = first snapshot, no previous); formula: `spent(N) = prev_nw(N-1) + income(N) - _snap_total_incl(N)`; `prev_nw` stores `_snap_total_incl` values. **Both sides of the spending formula are portfolio-inclusive** — market swings still affect the "spent" number, but the formula is at least internally consistent. Stats from confirmed months only; filter: `_cashflow_filter` (default "All")
