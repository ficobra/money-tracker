...
### Dashboard
- **Metric cards**: NET WORTH, MONTHLY CHANGE, FIXED EXPENSES (3 static cards in `cards_frame`). Extra cards rendered by `_render_extra_cards()` into `_extra_cards_frame`: PORTFOLIO + ALLOCATION DONUT (conditional), LAST MONTH INCOME (conditional). LAST MONTH INCOME shows income from `latest` (snapshots[0]); pct-change compares to `prev_snap` (snapshots[1]); label reads "vs {MONTHS[prev_snap['month']-1]}". `has_income_data = bool(latest)`. **Monthly change uses portfolio-inclusive totals**: `_render_comparison()` computes `latest_total = latest["total"] + portfolio_eur` and `prev_total = prev["total"] + portfolio_eur` before calculating change/pct. AVG/MO (`_total_incl` helper) and sparkline bars (`_snap_total` helper) also use portfolio-inclusive totals.
