...
from utils import fmt_eur, center_on_parent, lock_scroll, unlock_scroll, open_dialog, bind_numeric_entry
...
class ExpensesView(ctk.CTkScrollableFrame):
    ...
    def _build(self):
        ...
        self._expenses_error_lbl = ctk.CTkLabel(
            exp_hdr, text="", text_color=_RED, font=ctk.CTkFont(family=_F, size=12),
        )
        ...
        # Add expense form
        add_card = ctk.CTkFrame(self, fg_color=_BG_CARD, corner_radius=14,
                                border_width=1, border_color=_BORDER)
        add_card.pack(fill="x", padx=24, pady=(0, 16))
        form = ctk.CTkFrame(add_card, fg_color="transparent")
        form.pack(anchor="w", padx=20, pady=16)

        ctk.CTkLabel(form, text="Day", anchor="w", text_color=_TEXT_PRI).pack(side="left", padx=(0, 4))
        ...
        ctk.CTkLabel(form, text="Name", anchor="w", text_color=_TEXT_PRI).pack(side="left", padx=(0, 4))
        self._add_name = ctk.CTkEntry(form, placeholder_text="Expense name", width=220,
                                      fg_color=_BG_ELEM, border_color=_BORDER, text_color=_TEXT_PRI)
        self._add_name.pack(side="left", padx=(0, 14))
        ctk.CTkLabel(form, text="EUR", anchor="w", text_color=_TEXT_PRI).pack(side="left", padx=(0, 4))
        ...
        ctk.CTkButton(form, text="Add", width=80,
                      fg_color=_ACCENT, hover_color="#0096b4",
                      text_color="white", corner_radius=8,
                      command=self._add_expense).pack(side="left", padx=(0, 12))
        self._add_status = ctk.CTkLabel(form, text="", text_color=_TEXT_SEC)
        ...
        # Monthly total
        ctk.CTkFrame(self, height=1, fg_color=_BORDER).pack(fill="x", padx=24, pady=(16, 10))
        ...
        # ══ Monthly Income ════════════════════════════════════════════════════
        ctk.CTkFrame(self, height=1, fg_color=_BORDER).pack(fill="x", padx=24, pady=(0, 20))
        ...
        self._income_error_lbl = ctk.CTkLabel(
            inc_hdr, text="", text_color=_RED, font=ctk.CTkFont(family=_F, size=12),
        )
        ...
        ctk.CTkLabel(inc_form, text="EUR", anchor="w",
                     text_color=_TEXT_PRI).pack(side="left", padx=(0, 4))
        ...
        ctk.CTkButton(inc_form, text="Add", width=80,
                      fg_color=_ACCENT, hover_color="#0096b4",
                      text_color="white", corner_radius=8,
                      command=self._add_income_item).pack(side="left", padx=(0, 12))
        self._inc_add_status = ctk.CTkLabel(inc_form, text="", text_color=_TEXT_SEC)
        ...
        ctk.CTkLabel(months_row, text="Active months:", text_color=_TEXT_SEC,
                     font=ctk.CTkFont(family=_F, size=12)).pack(side="left", padx=(0, 8))
        for m in range(1, 13):
            ctk.CTkCheckBox(
    ...
    def _render_expense_row(self, exp: dict):
        ...
        if self._expenses_edit_mode:
            ...
            ctk.CTkEntry(row, textvariable=day_var, width=_W_DAY,
                         fg_color=_BG_ELEM, border_color=_BORDER,
                         text_color=_TEXT_PRI).pack(side="left", padx=(0, 6))
            ctk.CTkEntry(row, textvariable=name_var, width=_W_NAME,
                         fg_color=_BG_ELEM, border_color=_BORDER,
                         text_color=_TEXT_PRI).pack(side="left", padx=(0, 6))
            exp_amt_entry = ctk.CTkEntry(row, textvariable=amount_var, width=_W_AMOUNT,
                                         fg_color=_BG_ELEM, border_color=_BORDER,
                                         text_color=_TEXT_PRI)
            exp_amt_entry.pack(side="left", padx=(0, 10))
            bind_numeric_entry(exp_amt_entry)
            ctk.CTkButton(
            ...
            ctk.CTkLabel(row, text=str(exp["day_of_month"]), width=_W_DAY,
                         anchor="w", text_color=_TEXT_PRI).pack(side="left")
            ctk.CTkLabel(row, text=exp["name"], width=_W_NAME,
                         anchor="w", text_color=_TEXT_PRI).pack(side="left")
            ctk.CTkLabel(row, text=fmt_eur(exp["amount"]), width=_W_AMOUNT,
                         anchor="e", text_color=_TEXT_PRI).pack(side="left")
    ...
    def _render_income_display_row(self, item: dict):
        ...
        ctk.CTkLabel(row, text=item["name"], width=_W_INC_NAME,
                     anchor="w", text_color=_TEXT_PRI).pack(side="left")
        ctk.CTkLabel(row, text=months_display, width=_W_INC_MONTHS, anchor="w",
                     text_color=_TEXT_SEC, font=ctk.CTkFont(family=_F, size=11)).pack(side="left")
        ctk.CTkLabel(row, text=fmt_eur(item["amount"]), width=_W_INC_AMOUNT,
                     anchor="e", text_color=_TEXT_PRI).pack(side="left")

    def _render_income_edit_row(self, item: dict):
        outer = ctk.CTkFrame(self._income_list_frame, fg_color=_BG_ELEM, corner_radius=8)
        outer.pack(anchor="w", pady=2, fill="x")
        inner = ctk.CTkFrame(outer, fg_color="transparent")
        inner.pack(anchor="w", padx=12, pady=8, fill="x")
        ...
        line1.pack(anchor="w")
        ctk.CTkEntry(line1, textvariable=name_var, width=260,
                     fg_color="#161b22", border_color=_BORDER,
                     text_color=_TEXT_PRI).pack(side="left", padx=(0, 8))
        ctk.CTkLabel(line1, text="EUR", text_color=_TEXT_SEC).pack(side="left", padx=(0, 4))
        inc_edit_amt = ctk.CTkEntry(line1, textvariable=amount_var, width=110,
                                    fg_color="#161b22", border_color=_BORDER,
                                    text_color=_TEXT_PRI)
        inc_edit_amt.pack(side="left", padx=(0, 10))
        bind_numeric_entry(inc_edit_amt)
        ctk.CTkButton(
            line1, text="×", width=36, height=32,
            fg_color="#161b22", hover_color="#3d1a1a",
            text_color=_RED, corner_radius=8,
        ...
        ctk.CTkLabel(line2, text="Active months:", text_color=_TEXT_SEC,
                     font=ctk.CTkFont(family=_F, size=11)).pack(side="left", padx=(0, 6))
        for m in range(1, 13):
            ctk.CTkCheckBox(
                line2, text=_MONTHS_SHORT[m - 1], width=54,
                variable=month_vars[m], text_color=_TEXT_PRI,
            ).pack(side="left", padx=(0, 2))
    ...
    def _delete(self, expense_id: int, name: str):
        if not self._confirm_dialog(
            f'Are you sure you want to delete "{name}"?', confirm_text="Delete"
        ):
            return
    ...
    def _confirm_dialog(
        ...
        result = [False]
        dialog = open_dialog(self, 440, 150)
